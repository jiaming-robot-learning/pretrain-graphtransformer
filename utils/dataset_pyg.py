...
class MoleculeDatasetG(InMemoryDataset):
    def __init__(self,
                 root,
                 #data = None,
                 #slices = None,
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_full':
            ...
            for d_path in downstream_dir:
                ...
                downstream_dataset = MoleculeDatasetG(d_path, dataset=dataset_name)
...
def merge_dataset_objs(dataset_1, dataset_2):
    ...
    new_dataset = MoleculeDatasetG(root='dataset/chembl_with_labels',
...
def create_all_datasets():
    ...
    for dataset_name in downstream_dir:
        ...
        if os.path.exists(root + "/processed/geometric_data_processed.pt"):
            os.remove(root + "/processed/geometric_data_processed.pt")
        dataset = MoleculeDatasetG(root, dataset=dataset_name)
        print(dataset)


    dataset = MoleculeDatasetG(root = "dataset/chembl_filtered", dataset="chembl_filtered")
    print(dataset)
