...
def train_epoch(model, optimizer, device, data_loader, epoch,sup_ratio):
    ...
    for iter, (batch_graphs, batch_targets) in enumerate(data_loader):
        ...
        out = model.gnn(batch_graphs, batch_x, batch_e )
        ...
        # sup loss
        if sup_ratio > 0:
            sup_loss = model.gnn.loss(batch_scores, batch_targets)
...
def main():    
    ...
    parser.add_argument('--sup_ratio', help="Please give a value for sup_ratio",default=0)
    ...
    if args.exp_name is not None:
        ...
        exp_name = f'pretrain_gin_masking_{dataset_name}'
    ...
    pretrain(dataset, params, config, out_dir, exp_name)
